# 文档索引

| 文档 | 面向谁 | 内容 |
| --- | --- | --- |
| [research-agent-with-omp.md](research-agent-with-omp.md) | 全体 | 领域调研：AI Scientist v1/v2、Co-Scientist、PaperBench、MLE-bench、验证缺口；开源生态对照（§1.1）与借鉴清单（§7）；omp 能力映射与子agent 体系 |
| [architecture.md](architecture.md) | 贡献者 | 目录职责、数据流、omp 机制落点、设计取舍 |
| [experiment-protocol.md](experiment-protocol.md) | 实验执行者 | 预注册哈希、判据三态、seed、日志、状态推进、退出码、复核与写作闸门 |
| [project-preregistration.md](project-preregistration.md) | 全体 | 项目级主张与自注册 kill 判据：什么会杀死本项目、到期怎么判定 |
| [../CONTRIBUTING.md](../CONTRIBUTING.md) | 贡献者 | 本地开发、提交规范、评审要求 |

外部参考：

- Oh My Pi 机制文档：会话内读 `omp://tools/task.md`、`omp://tools/hub.md`、`omp://advisor-watchdog.md` 等。
- 引用与数字出处见 [research-agent-with-omp.md](research-agent-with-omp.md) 末尾的参考列表。
